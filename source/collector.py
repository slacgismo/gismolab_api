...
class Collector:
    ...
    def debug(self,msg):
        ...
        if DEBUG:
            print(f"DEBUG [{dt.datetime.now()} <collector:{self.get_name(6)}>]: {msg}",file=sys.stderr)

    def error(self,msg):
        """output an error message"""
        print(f"ERROR [{dt.datetime.now()} <collector:{self.get_name(6)}>]: {msg}",file=sys.stderr)

    def warning(self,msg):
        """output a warning message"""
        print(f"WARNING [{dt.datetime.now()} <collector:{self.get_name(6)}>]: {msg}",file=sys.stderr)
